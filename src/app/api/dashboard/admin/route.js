...
export async function GET(req) {
  ...
  try {
    ...
    const [usersData, departmentsData, pendingLeaves, pastLeaves, meData] = await Promise.all([
      ...
      // Past leaves (approved/rejected)
      Leave.find({ status: { $in: ['Approved', 'Rejected'] } })
        .populate('user', 'name email role leave_limit')
        .sort({ createdAt: -1 })
        .lean(),
    ...
    // Filter leaves (Admin sees Employee and HR leaves)
    const filteredPendingLeaves = pendingLeaves.filter(leave => 
      leave.user && (leave.user.role === 'Employee' || leave.user.role === 'HR')
    );
    const filteredPastLeaves = pastLeaves.filter(leave => 
      leave.user && (leave.user.role === 'Employee' || leave.user.role === 'HR')
    );
